...
public class SwaggerConfiguration {
  ...
  public Docket swaggerSpringfoxApiDocket() {
    ...
    Docket docket = new Docket(DocumentationType.SWAGGER_2)
        ...
        .protocols(new HashSet<>(Arrays.asList(applicationProperties.getSwagger().getProtocols())))
        .apiInfo(apiInfo)
  ...
  public Docket swaggerSpringFoxManagementDocket(
    ...
    return new Docket(DocumentationType.SWAGGER_2)
        ...
        .securitySchemes(Arrays.asList(securityScheme()))
        .securityContexts(
            Arrays.asList(securityContextForManagement(managementContextPath + "/.*")));
  }

  @Bean
  SecurityConfiguration security() {
    return SecurityConfigurationBuilder.builder()
        .clientId(applicationProperties.getSwagger().getOauthClientId())
        .clientSecret(applicationProperties.getSwagger().getOauthClientSecret())
        .scopeSeparator(" ")
  ...
  private SecurityScheme securityScheme() {
    GrantType grantType = new ResourceOwnerPasswordCredentialsGrant(
        applicationProperties.getSwagger().getAccessTokenUri());
  ...
  private AuthorizationScope[] scopes() {
    AuthorizationScope[] scopes = {
        new AuthorizationScope(ClientScopesConstants.ALL, "for all operation.")};
    return scopes;
  }

  private SecurityContext securityContextForManagement(String management) {
    return SecurityContext.builder()
        .securityReferences(Arrays.asList(new SecurityReference("spring_oauth", scopes())))
        .forPaths(PathSelectors.regex(management))
        .build();
  }
  ...
  private SecurityContext securityContext() {
    return SecurityContext.builder()
        .securityReferences(Arrays.asList(new SecurityReference("spring_oauth", scopes())))
        .forPaths(
            PathSelectors.regex(applicationProperties.getSwagger().getDefaultIncludePattern()))
        .build();
  }
}
